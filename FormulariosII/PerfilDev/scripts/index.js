...
//alternativa a: document.querySelctor('form')
...
function peticionSimulada() {
  loader.classList.remove('hidden');
  error.classList.add('hidden');

  setTimeout(function(){
    console.log("peticion simulada");
    loader.classList.add('hidden');
    validarCampos(email.value, password.value)
  }, 3000);
}
...
function validarCampos(usuario, contrasenia) {
  ...
  if(usuario.includes('@') && contrasenia.length>4){
    console.log("Correcto");
    error.classList.add('hidden');
    /* -------- recorriendo el listado(falsa base de datos) y chequeando -------- */
    baseDeDatos.forEach(user=>{
      if(user.email == usuario && user.password == contrasenia){
